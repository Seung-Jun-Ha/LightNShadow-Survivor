...
**New files to add:**
```
codex/skills/vertical-slice/SKILL.md
CONTRIBUTING.md
SECURITY.md
```

**Existing files to overwrite (no user content):**
- All files under `codex/skills/` modified in the commit range (skill audit + AskUserQuestion widgets + `--review` argument-hints)
...
| **Model routing** | GPT-5.4-Mini/GPT-5.4/GPT-5.5 tier assignments documented in coordination rules; skills declare their tier in frontmatter |
...
Skills are now explicitly assigned to GPT-5.4-Mini, GPT-5.4, or GPT-5.5 tiers based on task complexity. Read-only status checks use GPT-5.4-Mini; complex multi-document synthesis uses GPT-5.5; everything else defaults to GPT-5.4. Tier assignments are documented in `codex/docs/coordination-rules.md`.
...
```
ctx: 42% | Codex-GPT-5.4-4-6 | Systems Design
```
...
```
ctx: 42% | Codex-GPT-5.4-4-6 | Production | Combat System > Melee Combat > Hitboxes
```
...
이 파일은 Codex Game Studio Framework에서 '.\UPGRADING.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
