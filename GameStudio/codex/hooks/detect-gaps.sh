...
if [ "$FRESH_PROJECT" = true ]; then
  ...
  echo ""
  echo "?뮕 To get a comprehensive project analysis, run: /project-stage-detect"
  echo "==================================="
  exit 0
fi
...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  echo "?좑툘  GAP: Substantial codebase ($SRC_FILES source files) but sparse design docs ($DESIGN_FILES files)"
...
if [ -d "prototypes" ]; then
  ...
  if [ -n "$PROTOTYPE_DIRS" ]; then
    ...
    if [ ${#UNDOCUMENTED_PROTOS[@]} -gt 0 ]; then
      echo "?좑툘  GAP: ${#UNDOCUMENTED_PROTOS[@]} undocumented prototype(s) found:"
...
if [ -d "src/core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    echo "?좑툘  GAP: Core engine/systems exist but no docs/architecture/ directory"
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      echo "?좑툘  GAP: Core systems exist but only $ADR_COUNT ADR(s) documented"
    ...
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        ...
        if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ]; then
          echo "?좑툘  GAP: Gameplay system 'src/gameplay/$system_name/' ($file_count files) has no design doc"
...
if [ "$SRC_FILES" -gt 100 ]; then
  ...
  if [ ! -d "production/sprints" ] && [ ! -d "production/milestones" ]; then
    echo "?좑툘  GAP: Large codebase ($SRC_FILES files) but no production planning found"
...
exit 0
# 한국어 설명
# 이 파일은 Codex Game Studio Framework에서 '.\codex\hooks\detect-gaps.sh' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다.
