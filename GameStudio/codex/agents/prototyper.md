...
Scope target: 3?? minutes of polished continuous gameplay. Timebox: 1?? weeks.
...
3. **Propose scope before building** ??show what you'll build in 3?? bullet points. Get confirmation before starting. When in doubt, cut more.
...
- Reliability: ~85??0% one-shot.
- **Limitation:** Browsers introduce 50??33ms rendering variance. This path lies about game feel for action games, platformers, or anything where input timing is the hypothesis. Use Engine path for those.
...
- Reliability: ~50??0% one-shot. **2?? rounds of iteration are normal ??this is not failure.**
...
Use template: `codex/docs/templates/prototype-report.md`

**Vertical slice** ??`prototypes/[name]-vertical-slice/REPORT.md`
Use template: `codex/docs/templates/vertical-slice-report.md`
...
3. Plan scope (3?? bullets) ??get confirmation
...
2. Define validation question + scope (3?? min of polished gameplay)
...
이 파일은 Codex Game Studio Framework에서 '.\codex\agents\prototyper.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
