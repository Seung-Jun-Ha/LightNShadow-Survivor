...
> > - Slopes up to 45째 walked normally; >45째 blocked
...
> | `run_multiplier` | float | Run speed = walk 횞 this |
> | `terrain_speed_mod` | float | Per-terrain multiplier (0.7??.1) |
...
> **Effective Speed:**
> ```
> effective_speed = base_walk_speed 횞 run_multiplier (if running, else 1.0) 횞 terrain_speed_mod
> ```
>
> **Stamina Drain (per second):**
> ```
> drain_per_sec = stamina_drain_walk (or stamina_drain_run if running) 횞 terrain_drain_mod
> ```
>
...
> - [ ] Player runs at `base_walk_speed 횞 run_multiplier` on normal ground
...
**Agent:**
> Added:
> - [ ] Slopes ??5째 traversed normally; slopes >45째 block movement (player slides back)
>
> Write to file?
...
이 파일은 Codex Game Studio Framework에서 '.\docs\examples\session-design-system-skill.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
