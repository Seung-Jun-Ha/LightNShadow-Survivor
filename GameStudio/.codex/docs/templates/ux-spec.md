...
> **Related GDDs**: [Links to the GDD sections that generated this UI requirement ??e.g., `design/gdd/inventory.md 짠 UI Requirements`]
...
```
[Root ??e.g., Main Menu]
  ?붴?? [Parent Screen ??e.g., Settings]
        ?붴?? [This Screen ??e.g., Audio Settings]
              ?쒋?? [Child Screen ??e.g., Advanced Audio Options]
              ?붴?? [Child Screen ??e.g., Speaker Test Dialog]
```
...
?쒋????????????????????????????????????????????????
???뚢???????????????? ?뚢??????????????????????????먥봻
????CATEGORY NAV ?? ?? ITEM DETAIL PANEL      ?귘봻  ??CONTENT ZONE
???? ??Weapons   ?? ?? Item Name              ?귘봻
????   Armor     ?? ?? {item icon}            ?귘봻
????   Consumable?? ?? Stats comparison       ?귘봻
????   Key Items ?? ?? Description text...    ?귘봻
???쒋???????????????? ?붴??????????????????????????섃봻
????ITEM GRID    ??                            ??
????{??{??{??{?? ??                            ??
????{??{??{??{?? ??                            ??
...
That is 5 concurrent streams ??within the standard 7짹2 limit, but at the higher end.
...
> translation. German text is typically 30??0% longer than English. Arabic and Hebrew
...
| [e.g., Item description] | [~80??20 chars] | [200 chars] | [67%] | [Right-align, wrap normally] | [Scroll within Detail Panel ??no truncation] | [Low ??panel is scrollable] |
| [e.g., Action button "Equip"] | [5 chars] | [14 chars] | [180%] | [Button layout mirrors; text right-aligns] | [Shrink font to 90% minimum, then truncate] | [Medium ??"Ausr체sten" in German is 9 chars] |
...
- [ ] Navigation between items produces no perceptible frame drop (maintain target framerate 짹5fps)
...
이 파일은 Codex Game Studio Framework에서 '.Codex\docs\templates\ux-spec.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
