...
| Subtitles ??style customization | Comprehensive | Subtitle display | Not Started | Font size (4 sizes minimum), background opacity (0??00%), text color (white / yellow / custom), position (bottom / top / player-relative). |
...
| Aim assist | Standard | All ranged combat / targeting | Not Started | Not just on/off ??provide granularity: Assist Strength (0??00%), Assist Radius, Aim Magnetism (snap-to-target), and Aim Slowdown (near-target deceleration) as separate sliders. Default values should be tuned to feel helpful, not intrusive. |
...
| Independent volume controls | Basic | Music / SFX / Voice / UI audio buses | Not Started | Four independent sliders minimum. Persist to player profile. Range: 0??00%, default 80%. Expose in both main settings and the pause menu. |
...
이 파일은 Codex Game Studio Framework에서 '.Codex\docs\templates\accessibility-requirements.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
