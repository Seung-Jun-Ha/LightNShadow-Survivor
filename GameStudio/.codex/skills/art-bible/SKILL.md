...
- Tab **"Scope"** ??"Which sections need to be authored today?"
  Options: `Full bible ??all 9 sections` / `Visual identity core (sections 1?? only)` / `Asset standards only (section 8)` / `Resume ??fill in missing sections`
...
## Phase 2: Visual Identity Foundation (Sections 1??)
...
**Goal**: A one-line visual rule plus 2?? supporting principles that resolve visual ambiguity.
...
- Ask: "Draft a Visual Identity Statement for this game. Provide: (1) a one-line visual rule that could resolve any visual decision ambiguity, (2) 2?? supporting visual principles, each with a one-sentence design test ('when X is ambiguous, this principle says choose Y'). Anchor all principles directly in the stated pillars ??each principle must serve a specific pillar."
...
- Atmospheric descriptors (3?? adjectives)
...
Cover:
- Primary palette (5?? colors with roles ??not just hex codes, but what each color means in this world)
...
## Phase 3: Production Guides (Sections 5??)
...
**Agent delegation**: Spawn `art-director` via Task with sections 1??. Ask: "Define character design direction for this game. Cover: visual archetype for the player character (if any), distinguishing feature rules per character type (how do players tell enemies/NPCs/allies apart at a glance?), expression/pose style targets (stiff/expressive/realistic/exaggerated), and LOD philosophy (how much detail is preserved at game camera distance?)."

Write the approved section to file.

### Section 6: Environment Design Language

**Agent delegation**: Spawn `art-director` via Task with sections 1??. Ask: "Define the environment design language for this game. Cover: architectural style and its relationship to the world's culture/history, texture philosophy (painted vs. PBR vs. stylized ??why this choice for this game?), prop density rules (sparse/dense ??what drives the choice per area type?), and environmental storytelling guidelines (what visual details should tell the story without text?)."
...
이 파일은 Codex Game Studio Framework에서 '.Codex\skills\art-bible\SKILL.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
