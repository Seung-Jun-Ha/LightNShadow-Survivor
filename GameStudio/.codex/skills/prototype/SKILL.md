...
- **Options**:
  - `Prototype this concept` ??build a throwaway build to validate the core idea is fun before writing GDDs (1?? days)
...
**Reliability:** ~85??0% one-shot. The agent writes a single self-contained HTML
file the user opens in a browser ??no install required.

**Limitation ??browser latency lies about game feel.** Browsers introduce
50??33ms of rendering variance. This makes HTML prototypes fundamentally unreliable
...
**Reliability:** ~50??0% one-shot. Expect 2?? rounds of iteration ??this is
...
- **Options**:
  - `HTML ??browser prototype` ??puzzle, card, turn-based, strategy, idle. Opens by double-clicking, no install. 85??0% reliable. **Not suitable for action games** ??browser latency lies about feel.
  - `Engine ??native prototype` ??action, platformer, physics, or anything where feel IS the hypothesis. 50??0% one-shot; 2?? iteration rounds are normal. Requires engine installed.
...
Define in 3?? bullet points the minimum viable prototype:
...
If 2+ boxes apply ??KILL verdict is sound. If 0?? apply ??consider one more focused PIVOT before killing.
...
**Spike Mode workflow (replaces Phases 1??):**
...
이 파일은 Codex Game Studio Framework에서 '.Codex\skills\prototype\SKILL.md' 경로가 담당하는 원문 지침과 참조 정보를 보존합니다. Codex 환경에서는 AGENTS.md, codex/ 지식 베이스, Unity 우선 엔진 참조를 기준으로 읽으면 됩니다. 핵심 결정은 원문을 삭제하지 않고, Codex 기준 경로와 Unity 제작 흐름을 함께 이해하도록 보조 설명을 추가하는 것입니다.
