...
# Codex Game Studio Framework -- Agent Instructions

This repository now also includes the Codex-oriented Game Studio framework from `SeongHuni/Game-Studio`. Codex should treat this `AGENTS.md` as the primary workspace instruction file and use `codex/` as the framework knowledge base, while preserving the Unity project guidance above for local development.
...
- **Build system**: Unity Build Profiles or project-specific CI
- **Asset pipeline**: Unity Addressables, import presets, and explicit asset validation

## Codex Workspace Map

- `AGENTS.md`: root instructions for Codex in this repository
- `codex/agents/`: specialist role prompts that Codex can use as delegation guidance
- `codex/skills/`: Codex skill-style workflow instructions
- `codex/rules/`: path-scoped implementation standards
- `codex/docs/`: framework references, templates, and workflow documentation
- `docs/engine-reference/unity/`: pinned Unity API and best-practice references
- `src/AGENTS.md`, `docs/AGENTS.md`, `design/AGENTS.md`: directory-local instructions

Legacy `CLAUDE.md` files may remain for migration context, but new Codex work should prefer `AGENTS.md` and `codex/` references.

## Operating Rules

- Read the nearest `AGENTS.md` before editing files in a subdirectory.
- Prefer Unity-specific agents, skills, and engine references unless the user explicitly chooses Godot or Unreal.
- Use existing framework templates and local conventions before inventing new structure.
- Keep edits scoped to the requested workflow, document, agent, skill, or Unity support surface.
- Do not commit, push, or publish without a direct user request.
- When implementation details are missing, make a conservative assumption and state it, unless the choice can damage data or create broad churn.

## Collaboration Protocol

Codex should be implementation-ready, not approval-gated for every file edit. For ordinary requested edits, make the change, verify it, and summarize the result. Ask before writing only when the change is destructive, ambiguous in a high-impact way, touches secrets, or expands beyond the user's stated scope.

## Unity Defaults

- Use `docs/engine-reference/unity/` before relying on engine APIs.
- Prefer data-driven gameplay values, ScriptableObjects for authoring data, and Addressables for runtime asset loading.
- Avoid `Find`, `FindObjectOfType`, `SendMessage`, per-frame allocations, and `GetComponent` in hot paths.
- Add tests for gameplay formulas, save/load behavior, deterministic systems, and integration boundaries where practical.
